...
Singleton {
    ...
    readonly property alias values: themeAdapter

    FileView {
        id: themeFile
        path: Qt.resolvedUrl("./theme_settings.json").toString().replace("file://", "")
        blockLoading: true
    ...
    function updateSetting(key, value) {
        if (themeAdapter.hasOwnProperty(key)) {
            themeAdapter[key] = value;
            themeFile.writeAdapter();
        } else {
            console.warn("Theme.qml: Property '" + key + "' does not exist.");
        }
    }

    function resetToDefaults() {
        var oldPath = themeFile.path;
        themeFile.path = "";
        themeFile.path = oldPath;
    }
}
